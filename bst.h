...
class tree_node //definition of node of the tree
   ...
   public:
       ...
       std::unique_ptr<tree_node> right;//Unique ptr to the right node from this node
       tree_node* up;//ptr to the proper node which key is next in the tree
...
{
  // initialisation of the compar operator
    cmp compar;
    ...
    std::pair<iterator,bool> insert (const std::pair<const key_type,value_type>& x)
    {
       ...
       while(temp)
       {
           if(compar(temp->get_key(),x.first))
           {
               ...
               {                //verificare se è temp o temp->get_up()
